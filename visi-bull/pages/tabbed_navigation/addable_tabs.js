...
{
    // Create the tabs.
    var textForTabs = ["Iron Cook", "Hey, what kinda party is this? There's no booze and only one hooker.", "Zinc Saucier", "How come you always dress like you're doing your laundry?"];
    ...
    create_moveable_tabs("#addable-tabs");
    ...
    function create_moveable_tabs(tabSetID)
    {
        ...
        var addTabButtonDiameter = 15;  // The diameter of the of the add tab button.
        var addTabCrossSize = 8;  // The width and height of the add tab button plus sign.
        ...
        var newTabContainer = tabSet.append("g")
            .datum({"transX" : numberOfTabs * (curveWidth + tabWidth) + curveWidth, "transY" : svgHeight - tabHeight})
            .classed("add-button", true)
            .attr("transform", function(d) { return "translate(" + d.transX + "," + d.transY + ")"; });
        newTabContainer.append("circle")
            .attr("cx", addTabButtonDiameter / 2)
            .attr("cy", tabHeight / 2)
            .attr("r", addTabButtonDiameter / 2);
        newTabContainer.append("path")
            .attr("d", "M" + ((addTabButtonDiameter - addTabCrossSize) / 2) + "," + (tabHeight / 2) +
                       "h" + addTabCrossSize +
                       "M" + (addTabButtonDiameter / 2) + "," + ((tabHeight / 2) - (addTabCrossSize / 2)) +
                       "v" + addTabCrossSize);
        ...
        var tabContainer = tabSet.selectAll(".tab-container")
            ...
            .append("g")
            .attr("transform", function(d) { return "translate(" + d.transX + "," + d.transY + ")"; })
            .attr("id", function(d) { return "tab-container-add-" + d.key; })
        ...
        var clips = tabContainer.append("clipPath")
            .attr("id", function(d) { return "clip-add-" + d.key; });
        clips.append("path")
            ...
            .attr("d", function(d)
                {
                    ...
                    {
                        config.leftTabXPos = tabSet.select("#clip-add-" + (keyToPosition[d.key] - 1)).datum().transX;
                    }
                    return create_clip_tab_style_1(config, d.transX);
                });
        tabContainer.attr("clip-path", function(d) { return "url(#clip-add-" + d.key + ")"; });
            ...
            .on("click", function(d)
                {
                    ...
                    var tabToClose = tabSet.select("#tab-container-add-" + d.key);
                    ...
                    for (var i = 0; i < numberOfTabs; i++)
                    {
                        if (posOfClickedTab < i)
                        {
                            ...
                            var currentTab = tabSet.select("#tab-container-add-" + currentKey);
                    ...
                    // Transition the new tab button.
                    newTabContainer
                        .transition()
                        .duration(transitionDuration)
                        .ease("linear")
                        .attr("transform", function(d) { d.transX -= (tabWidth + curveWidth); return "translate(" + d.transX + "," + d.transY + ")"; });
        ...
        var patterns = tabIcons.append("pattern")
            .attr("id", function(d) { return "icon-image-add-" + d.key; })
            .attr("x", 0)
            .attr("y", 0)
        ...
        tabIcons.append("rect")
            ...
            .attr("width", tabIconSize)
            .attr("height", tabIconSize)
            .style("fill", function(d) { return "url(#icon-image-add-" + d.key + ")"});

        // Add the text to the tabs.
        defs.append("clipPath")
            .attr("id", "clip-text-add")
            .append("rect")
                .attr("x", 0)
                .attr("y", 0)
        ...
        var tabTextElements = tabContentContainer
            ...
            .attr("clip-path", "url(#clip-text-add)")
        ...
        tabTextElements.each(function(d, i)
            {
                ...
                if (currentTextWidth > maxTextWidth)
                {
                    ...
                    // Set the fade.
                    d3.select(this)
                        .style("fill", "url(#fadeGradient-add-" + i + ")");
                }
            });
            ...
            tabSet.selectAll(".clip-tab").attr("d", function(clipD)
                {
                    ...
                    else if (currentTabPos === numberOfTabs - 1)
                    {
                        ...
                        config.leftTabXPos = tabSet.select("#tab-container-add-" + positionToKey[currentTabPos - 1]).datum().transX;
                    }
        ...
        *********************/
        ...
        function drag_update(d)
        {
            ...
            var leftTab = tabSet.select("#tab-container-add-" + leftTabKey);
            var rightTabKey = positionToKey[selectedTabPos + 1];
            var rightTab = tabSet.select("#tab-container-add-" + rightTabKey);
